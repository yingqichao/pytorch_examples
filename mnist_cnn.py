...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        ### Let Entropy Shrink? Added:2020.7.25
        ...
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} which contains {:2.2f}% Entropy Loss (Entropy: {:2.4f})'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader), loss.item(),100*loss_entropy/loss.item(),entropy))
            if args.dry_run:
                break
